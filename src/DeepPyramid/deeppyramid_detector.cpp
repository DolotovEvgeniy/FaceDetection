...
#include <DeepPyramid/deeppyramid_detector.hpp>
#include <nms.hpp>
...
DeepPyramidDetector::DeepPyramidDetector(FileStorage config) {
    string model_file;
...
void DeepPyramidDetector::extractFeatureMap(const Mat &img,
                                            vector<Rect>& objects,
                                    const Size& size,
...
Size DeepPyramidDetector::embeddedImageSize(const Size& imgSize,
...
void DeepPyramidDetector::constructImagePyramid(const Mat& img,
...
void DeepPyramidDetector::
constructFeatureMapPyramid(const vector<Mat>& imgPyramid,
    ...
    for (size_t i = 0; i < levelCount; i++) {
        ...
        net->processImage(imgPyramid[i], data);
        map.normalize();
        maps[i] = map;
    }
}
...
void DeepPyramidDetector::
processFeatureMap(const int& filterIdx, const FeatureMap& map,
    ...
    for (int x = 0; x < xEnd; x+=stride) {
        for (int y = 0; y < yEnd; y+=stride) {
            ...
            if (filter.predictObjectType(rectMap) == OBJECT) {
                ...
                box.feature
                        = shared_ptr<BoxFeature>(new FeatureMap(rectMap));
...
void DeepPyramidDetector::detect(const vector<FeatureMap>& maps,
                         vector<BoundingBox>& detectedObjects) const {
...
double DeepPyramidDetector::imageScale(const int& level) const{
    return pow(2, (levelCount-1-level)/2.0);
}

Rect DeepPyramidDetector::norm5Rect2Original(const Rect& norm5Rect,
                                             const int& level,
...
Rect DeepPyramidDetector::originalRect2Norm5(const Rect& originalRect,
                                     const int& level,
                                     const Size& imgSize) const {
    double longSide = std::max(imgSize.height, imgSize.width);
...
int DeepPyramidDetector::chooseLevel(const Size& filterSize,
                                     const Rect& boundBox,
                             const Size& imgSize) const {
    vector<double> f;
...
void DeepPyramidDetector::
calculateOriginalRectangle(vector<BoundingBox>& detectedObjects,
                                             const Size& imgSize) const {
    for (size_t i = 0; i < detectedObjects.size(); i++) {
        Rect originalRect
                = norm5Rect2Original(detectedObjects[i].norm5Box,
                                     detectedObjects[i].level, imgSize);
        detectedObjects[i].rect = originalRect;
    }
}

void DeepPyramidDetector::
groupRectangle(vector<BoundingBox>& detectedObjects) const {
    NMSweightedAvg nms;
    nms.processBondingBox(detectedObjects, BOX_THRESHOLD,
                                           CONFIDENCE_THRESHOLD);
}

void DeepPyramidDetector::detect(const Mat& img, vector<Rect>& detectedObjects,
                         vector<float>& confidence) const {
    ...
    for (size_t i = 0; i < objects.size(); i++) {
        detectedObjects.push_back(objects[i].rect);
